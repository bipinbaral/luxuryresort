    ...
    <div>
        ...
        <div className="max-w-screen-xl mx-auto">
          <Reveal>
            <div
              ...
              style={{ border: '1px solid rgba(138,114,86,0.2)' }}
            >
              <div className="img-hover" style={{ height: '480px', background: '#C8AD8F' }}>
                <img
              ...
              <div className="flex flex-col justify-center p-8 md:p-12" style={{ background: '#FAF8F3' }}>
                <div className="flex items-center gap-4 mb-5">
                  ...
                  <span className="text-xs" style={{ color: '#968B7F' }}>{JOURNAL_POSTS[0].readTime}</span>
                </div>
                <h2 className="display" style={{ fontSize: 'clamp(24px, 3vw, 40px)', lineHeight: 1.15 }}>
                  {JOURNAL_POSTS[0].title}
                </h2>
                <p className="mt-4 leading-relaxed" style={{ fontSize: '14px', color: '#968B7F', lineHeight: 1.8 }}>
                  {JOURNAL_POSTS[0].excerpt}
                </p>
                <div className="flex items-center gap-4 mt-8">
                  <div className="w-8 h-8 rounded-full bg-mist flex items-center justify-center text-xs font-medium" style={{ background: '#EBE6DD', color: '#8A7256' }}>
                    {JOURNAL_POSTS[0].author.split(' ')[0][0]}
                  </div>
                  <div>
                    <p className="text-xs font-medium" style={{ color: '#1C1A17' }}>{JOURNAL_POSTS[0].author}</p>
                    <p className="text-xs" style={{ color: '#968B7F' }}>{JOURNAL_POSTS[0].date}</p>
                  </div>
                </div>
            ...
            {JOURNAL_POSTS.slice(1).map((post, i) => (
              <Reveal key={post.slug} delay={i * 80}>
                <div className="group cursor-pointer">
                  <div className="img-hover" style={{ height: '280px', background: '#C8AD8F' }}>
                    <img
                      src={post.image}
                      alt={post.title}
                      style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
                  ...
                  <div className="pt-5 pb-5 border-b" style={{ borderColor: 'rgba(138,114,86,0.2)' }}>
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <span className="text-xs" style={{ color: '#968B7F' }}>{post.readTime}</span>
                    </div>
                    <h3 className="display" style={{ fontSize: '20px', lineHeight: 1.2 }}>{post.title}</h3>
                    <p className="mt-3 text-sm leading-relaxed" style={{ color: '#968B7F', lineHeight: 1.75 }}>
                      {post.excerpt.slice(0, 100)}…
                    </p>
                    <div className="flex items-center justify-between mt-5">
                      <p className="text-xs" style={{ color: '#968B7F' }}>{post.date}</p>
      ...
      <section className="py-20 px-6 text-center" style={{ background: '#EBE6DD' }}>
        <Reveal>
          ...
          <p className="mt-3 mb-8 text-sm max-w-sm mx-auto leading-relaxed" style={{ color: '#968B7F' }}>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center max-w-sm mx-auto">
            ...
            />
            <button className="btn-primary" style={{ background: '#253227', whiteSpace: 'nowrap' }}>
              Subscribe
            </button>
          </div>
        </Reveal>
